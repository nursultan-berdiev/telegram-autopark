   ...
   Traccar ◄─WS/REST─► tracker-adapter ┘        │  единственный писатель БД   │
                       (провайдеры трекеров)    └──────────────┬──────────────┘
                                                        ...
                                                        └──────┬──────┘
                                                               │
                                                      ┌────────▼────────┐
                                                      │  celery-worker  │──► carcheck.gov.kg
                                                      │  Playwright     │    (проверка штрафов)
                                                      └─────────────────┘
```
...
| `celery-worker` | Фоновые задачи, которым нужен браузер: проверка штрафов на carcheck.gov.kg (Playwright + Chromium в образе `Dockerfile.worker`). |
...
- **Штрафы: важен факт, а не сумма.** carcheck.gov.kg суммы не отдаёт вовсе — только тип,
  номер постановления и время. Бот сообщает, что штраф появился и сколько их; сумму оператор
  смотрит сам. Подробности и все выясненные опытом факты — в
  [plan/10](plan/10-fines-scheduling-admin.md).
- **Отказ сервиса ≠ «штрафов нет».** У прогона три исхода (`ok`, `refused`, `failed`), и
  заблокированная капчей проверка никогда не выглядит как успешная и пустая.
...
- [plan/10-fines-scheduling-admin.md](plan/10-fines-scheduling-admin.md) — carcheck.gov.kg (форма ответа, поведение капчи), расписания задач, веб-админка и её известные слабости
