...
def test_due_line_future():
    from bot.handlers.schedules import _due_line

    assert "Следующий платёж" in _due_line(_sched(7))


def test_due_line_today():
    from bot.handlers.schedules import _due_line

    line = _due_line(_sched(0))
    assert "Срок сегодня" in line
    assert "просрочен" not in line.lower(), "в день срока просрочки ещё нет"


def test_due_line_overdue_counts_days_and_debt():
    """Три пропущенных недели при аренде 1000/нед — долг 3000, а не 1000."""
    from bot.handlers.schedules import _due_line

    line = _due_line(_sched(-20))  # сроки наступили на -20, -13, -6 днях
    assert "просрочен на 20 дн." in line
    assert "3000.00" in line


def test_due_line_overdue_includes_todays_due():
    """Граница: ровно 21 день назад при недельном графике — сегодня НАСТУПИЛ 4-й срок.

    Сроки: -21, -14, -7 и сегодня. Сегодняшний платёж тоже подлежит уплате,
    поэтому долг 4000, а не 3000 — это не ошибка счёта, а совпадение дат.
    """
    from bot.handlers.schedules import _due_line

    assert "4000.00" in _due_line(_sched(-21))
