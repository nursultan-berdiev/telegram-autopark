...
from app.db.models import Car, Driver, Payment, PaymentSchedule
from app.domain.schedules import due_summary, fmt_money, schedule_status
...
async def build_snapshot(session: AsyncSession) -> str:
    ...
    overdue_items = [it for it in upcoming if it.overdue_days >= 1]
...
async def statement_by_car(session: AsyncSession) -> list[CarTotal]:
    ...
    return [
        CarTotal(car_id=car.id, plate=car.plate, total=float(total), count=int(count))
